import "@testing-library/jest-dom";
import { render } from "@testing-library/react";
import React from "react";

import { AcceleratorFeatureCard } from "../AcceleratorFeatureCard";

const imgData = {
  alternativeText: "Ingest",
  caption: "Ingest",
  localFile: {
    childImageSharp: {
      gatsbyImageData: {
        layout: "constrained",
        placeholder: {
          fallback:
            "data:image/png;base64,",
        },
        images: {
          fallback: {
            src: "/static/02a976acb55429127975095ec5ea926f/3fd26/feature_Img_01_f296b86174.png",
            srcSet:
              "/static/02a976acb55429127975095ec5ea926f/12411/feature_Img_01_f296b86174.png 99w,\n/static/02a976acb55429127975095ec5ea926f/0b232/feature_Img_01_f296b86174.png 198w,\n/static/02a976acb55429127975095ec5ea926f/3fd26/feature_Img_01_f296b86174.png 396w",
            sizes: "(min-width: 396px) 396px, 100vw",
          },
          sources: [],
        },
        width: 396,
        height: 396,
      },
    },
  },
};

describe("Test if the AcceleratorFeatureCard component is working as expected", () => {
  it("<AcceleratorFeatureCard /> matches snapshot", () => {
    const component = render(
      <AcceleratorFeatureCard
        title="Ingest"
        logo={imgData}
        bgColor="white"
        isImage={true}
      />
    );
    expect(component.container).toMatchSnapshot();
  });

  it("<AcceleratorFeatureCard /> renders without exploding", () => {
    const component = render(
      <AcceleratorFeatureCard
        title="Ingest"
        logo={imgData}
        bgColor="white"
        isImage={true}
      />
    );
    expect(
      component.getByTestId("accelerator-feature-card")
    ).toBeInTheDocument();
  });
});
