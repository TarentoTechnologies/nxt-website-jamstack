import "@testing-library/jest-dom";
import { render } from "@testing-library/react";
import React from "react";

import { ClientCard } from "../ClientCard";

const imgData = {
  localFile: {
    childImageSharp: {
      gatsbyImageData: {
        layout: "constrained",
        placeholder: {
          fallback:
            "data:image/png;base64,",
        },
        images: {
          fallback: {
            src: "/static/02a976acb55429127975095ec5ea926f/3fd26/feature_Img_01_f296b86174.png",
            srcSet:
              "/static/02a976acb55429127975095ec5ea926f/12411/feature_Img_01_f296b86174.png 99w,\n/static/02a976acb55429127975095ec5ea926f/0b232/feature_Img_01_f296b86174.png 198w,\n/static/02a976acb55429127975095ec5ea926f/3fd26/feature_Img_01_f296b86174.png 396w",
            sizes: "(min-width: 396px) 396px, 100vw",
          },
          sources: [],
        },
        width: 396,
        height: 396,
      },
    },
  },
};

describe("Test if the ClientCard component is working as expected", () => {
  it("<ClientCard /> matches snapshot", () => {
    const component = render(<ClientCard image={imgData} />);
    expect(component.container).toMatchSnapshot();
  });

  it("<ClientCard /> renders without exploding", () => {
    const component = render(<ClientCard image={imgData} />);
    expect(component.getByTestId("client-card")).toBeInTheDocument();
  });
});
